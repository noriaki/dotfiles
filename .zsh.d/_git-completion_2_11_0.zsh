...
__git_config_value () {
  ...
  opt_args=()
  local -a line
  line=( ${words[CURRENT]%%=*} ${words[CURRENT]#*=} )
  local state=value
  __git_config_option-or-value "$@"
}
...
__git_config_option-or-value () {
  ...
  () {
    ...
    sections_that_permit_arbitrary_subsection_names=(
      alias
      pager
      pretty
      remotes
      ...
      if compset -P '[^.]##.*.'; then
        ...
        case $IPREFIX in
          ...
          (svn-remote.)
            ...
            ;;
          (*.)
            local -a existing_subsections
            existing_subsections=( ${${${(M)git_present_options:#${IPREFIX}*.*}#${IPREFIX}}%.*} )
